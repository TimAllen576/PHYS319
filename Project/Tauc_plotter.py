...
def scale_n_differentiate(datadf):                                  # refactor?
    ...
    for sample_name in datadf.columns[1:]:
        ...
        scaled_df["ahv_data",sample]=((
            ...
            :glass_int_index]
        # Smooth data and find its derivative using preferred filter
        scaled_df["data_deriv",sample] = smooth_n_deriv(
            scaled_df["ahv_data", sample],
            scaled_df["Energy","eV"],
            window, p_order)
...
def band_gap_calculator():
    """Calculates the band gap using Tauc-like plots as..."""
    #derivative rel max
...
class InteractivePlot:
    ...
    def __init__(self, scaled_df):
        ...
        self.window, self.p_order= 30, 1
        self.demo_data = pd.DataFrame(scaled_df.iloc[:,1])
        self.sample = self.demo_data.columns[0][1]
        ...
        self.win_size = Slider(
            ...
            valmin=1, valmax=99, valinit=30, valstep=1)
        self.ax.set_xlabel("Photon energy (eV)")
        self.ax.set_ylabel("Gradient")
        self.ax.grid(True)

        # Initial calculations
        self.demo_data["data_deriv_raw", self.sample] = np.gradient(
            self.demo_data["ahv_data", self.sample],
            self.energy)
        self.raw =self.ax.plot(self.energy,
                self.demo_data["data_deriv_raw",self.sample],
                "b.",markersize=1, alpha = 0.5)
        self.demo_data["data_deriv",self.sample] = smooth_n_deriv(
            self.demo_data["ahv_data", self.sample],
            self.energy,
            self.window, self.p_order)
        self.current,=self.ax.plot(self.energy,
                self.demo_data["data_deriv", self.sample],
                "r-",linewidth=0.5)
    ...
    def update_win(self,window):
        ...
        self.demo_data["data_deriv",self.sample] = smooth_n_deriv(
            ...
            self.energy,
            self.window, self.p_order)
        self.current.set_ydata(self.demo_data["data_deriv",self.sample])
...
def main():
    ...
    demo = InteractivePlot(scaled_data)
    demo.show()
